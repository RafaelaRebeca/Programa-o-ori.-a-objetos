...
class ManterPerfilUI:
    ...
    def inserir():
        ...
        bene = st.text_input("Informe os benefícios")
        if st.button("Inserir"):
            View.perfil_inserir(nome, desc, bene)
    ...
    def atualizar():
        ...
        if len(perfis) == 0: 
            ...
            desc = st.text_input("Informe o novo descrição", op.desc)
            bene = st.text_input("Informe o novo benefício", op.bene)
            if st.button("Atualizar"):
                View.perfil_atualizar(op.id, nome, desc, bene)
